Page({
  ...
  nextgame() {
    ...
    let flag = 0;
    this.setData({
      beilv: this.data.play[0].current + this.data.play[this.data.count%2].current + this.data.beilv
    })
    let t = parseInt((this.data.play[0].point - this.data.play[this.data.count%2].point) * this.data.beilv);
    let t0 = parseInt(this.data.play[0].chip) + t;
    let t1 = parseInt(this.data.play[this.data.count%2].chip) - t;
    if (t0 <= 0 || t1 <= 0) {
      flag = 1;
    }
    this.setData({
    ...
    if (this.data.playcount == 0 || flag == 1) {
      let name='';
      if(this.data.play[0].chip>this.data.play[1].chip){name='p1'}
      else if(this.data.play[0].chip<this.data.play[1].chip){name='p2'}
      else {name='p1&p2'}
      this.setData({
        winner:name
      })
    ...
    let r1 = this.data.play[this.data.count%2].suoding;
    for (let i = 0; i < 5; i++) {
      r0.pop()
      r1.pop()
    }
  ...
  checksmallsunzi(list) {
    ...
    if (Has1 && Has2 && Has3 && Has4) {
      for (let i = 1; i <= 4; i++) {
        if (index[i] < this.data.play[this.data.count%2].lock) {} else {
          this.locked(index[i])
        }
      }
      return true;
