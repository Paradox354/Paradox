Page({
  ...
  nextgame()
  {
    ...
    let flag=0;
    this.setData({
      beilv:this.data.play[2].current+this.data.play[0].current+this.data.play[1].current+this.data.beilv
    })
    for(let i=0;i<3;i++)
    { 
  ...
  countdown() {
    if (this.data.play[this.data.count % 3].lock == 5&&this.data.xiazhu==false) {
      this.setData({
    ...
    if (this.data.play[this.data.count % 3].isrobot && this.data.time == 10 && this.data.count % 6 != 5 && this.data.count % 6 != 4&& this.data.count % 6 != 3) {
      this.robot();
    }
    if (this.data.play[this.data.count % 3].isrobot && this.data.time == 7) {
      this.queding()
    }
    if (this.data.time == -1) 
    {
      if (this.data.count == 12||this.data.count==13||this.data.count==14) {
        if (!this.data.isbind) {
          this.bindtap()
        }
        this.alllock();
      ...
      this.sort_lock()
      this.data.time = 15;
      if (this.data.count == 2 || this.data.count == 3 || this.data.count == 4 || this.data.count == 8|| this.data.count == 9|| this.data.count == 10) {
        if (this.data.play[this.data.count % 3].isrobot) {
          this.robotxiazhu()
        }
        this.setData({
          isable: true,
      ...
      let i=parseInt((this.data.count+1)/6)+1;
      this.setData({
        count: this.data.count + 1,
        lunci:i
      })
  ...
  sort_lock: function () {
    ...
    let num = 0;
    for (var i = 0; i < 5; i++) {
      if((this.data.count==12||this.data.count==13||this.data.count==14)&&this.data.checkbord[i].data!=0)
      {
    ...
    for (let i = 0; i < num; i++) {
      let t = this.data.play[this.data.count % 3].lock++;
      let revise = "play[" + this.data.count % 3 + "].suoding[" + t + "]";
      this.setData({
        [revise]: list[i]
      })
    }
  ...
  bindPickerChange: function (e) {
    //let i=this.data.play[this.data.count%3].current+parseInt(e.detail.value)
    let i=parseInt(e.detail.value)
    this.setData({
      ["play["+this.data.count%3+"].current"]: i
    });
  },
  //摇骰子
  bindtap: function () {
    for (let i = 0; i < this.data.play[this.data.count % 3].lock; i++) {
      let revise = "checkbord[" + i + "].data";
      let ran = 0;
      this.setData({
        [revise]: ran
    ...
    for (let i = this.data.play[this.data.count % 3].lock; i < 5; i++) {
  ...
  //入口
  lock1: function () {
    if(this.data.play[this.data.count%3].isrobot){return ;}
    this.locked(0)
  },
  lock2: function () {
    if(this.data.play[this.data.count%3].isrobot){return ;}
...
locked: function (i) {
  ...
  get_point: function () {
    ...
    let sum=0;
    for (let i = 0; i < this.data.play[this.data.count % 3].lock; i++) {
      list[this.data.play[this.data.count % 3].suoding[i]]++;
      sum+=this.data.play[this.data.count % 3].suoding[i];
      if (list[this.data.play[this.data.count % 3].suoding[i]] == 1) {
        type++;
      }
    }
    if (this.data.play[this.data.count % 3].lock <= 2) {} else if (this.data.play[this.data.count % 3].lock == 3) {
      if (type == 1) {
        op = 10
      }
    } else if (this.data.play[this.data.count % 3].lock == 4) {
      if (type == 1) {
        op = 40
    ...
    let revise = "play[" + this.data.count % 3 + "].point";
    this.setData({
      [revise]: op+sum
    })
  },
  ...
  checksmallsunzi(list) {
    ...
    if (Has1 && Has2 && Has3 && Has4) {
      for (let i = 1; i <= 4; i++) {
        if (index[i] < this.data.play[this.data.count%3].lock) {} else {
          this.locked(index[i])
        }
      }
      return true;
  ...
  robotxiazhu() {
    let re = 'play[' + this.data.count % 3 + '].current'
    this.setData({
      [re]: this.data.ubeilv
    })
  },
  aiplay() {
    let re = 'play[' + this.data.count % 3 + '].isrobot'
    if (this.data.play[this.data.count % 3].isrobot) {
      let flag = false;
      this.setData({
        [re]:flag,
